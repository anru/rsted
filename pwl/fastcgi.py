...
to run Django apps from pwl servers that support the FastCGI protocol.
...
from pwl.utils import importlib
import sys
import os
...
def runfastcgi(argset=[], **kwargs):
    ...
    if options['method'] in ('prefork', 'fork'):
        wsgi_opts = {'maxSpare': int(options['maxspare']), 'minSpare': int(options['minspare']),
                     'maxChildren': int(options['maxchildren']), 'maxRequests': int(options['maxrequests'])}
        flup_module += '_fork'
